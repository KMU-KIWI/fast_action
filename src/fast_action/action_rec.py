...
def detection_inference(args, frame_paths):
    ...
    assert model is not None, (
        "Failed to build the detection model. Check if you have installed mmcv-full properly. "
...
def main():
    ...
    if GCN_flag:
        ...
        tracking_inputs = [
            [pose["keypoints"] for pose in poses] for poses in pose_results
        ]
        keypoint, keypoint_score = pose_tracking(
            tracking_inputs, max_tracks=GCN_nperson
        )
        ...
        keypoint_score = np.zeros(
            (num_person, num_frame, num_keypoint), dtype=np.float16
        )
